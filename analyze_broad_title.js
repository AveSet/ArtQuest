...
console.log('=== ALL ENGLISH WORDS IN title.ru (broader: any 3+ Latin letters) ===');
let total = 0;
for (const fname of files) {
    ...
    const cnt = data.filter(q => /[A-Za-z]{3,}/.test((q.title && q.title.ru) || '')).length;
    console.log('\n--- ' + fname + ' (' + cnt + ' quests with English in title.ru) ---');
    matched.forEach(m => console.log(m));
}
console.log('\nTOTAL title.ru with any English: ' + total);
